...
COPY --from=build /springboot/api/target/api-0.0.1-SNAPSHOT.jar api-0.0.1-SNAPSHOT.jar
COPY --from=build /springboot/elastic-apm-agent-1.19.0.jar elastic-apm-agent-1.19.0.jar

ENTRYPOINT ["java", "-javaagent:./elastic-apm-agent-1.19.0.jar", "-Delastic.apm.server_url=http://apmserver:8200", "-Delastic.apm.service_name=api", "-jar","api-0.0.1-SNAPSHOT.jar"]
